    ...
    let checkValid = () => {
        ...
        if (match) {
            ...
            if (mapid == parsedBeatmapID) {
                let map_obj = mappool.beatmaps.find(
                    (m) => m.beatmap_id == mapid
                );
                if (map_obj?.identifier?.toUpperCase().includes("TB"))
                    return -3;
                if (nameRed && nameBlue) {
                    $("#picked_by")
                        .text(
                            `Picked by ${
                                cookieValue[1] === "red" ? nameRed : nameBlue
                            }`
                        )
                        .css("opacity", 1)
                        .addClass(cookieValue[1])
                        .removeClass(opposite_team(cookieValue[1]));
                    $("#map_slot_container")
                        .addClass(cookieValue[1])
                        .removeClass(opposite_team(cookieValue[1]));
                    $("#map_image_container")
...
socket.onmessage = async (event) => {
        ...
        if (bestOf === undefined) {
            for (let i = 1; i <= newmax; i++) {
                $("#red_points").append(
                    $("<div></div>")
                        .attr("id", `red${i}`)
                        .addClass("team-point red")
                );
                $("#blue_points").append(
                    $("<div></div>")
                        .attr("id", `blue${i}`)
                        .addClass("team-point blue")
                );
            }
    ...
    if (update_stats) {
        ...
        map = mappool
            ? mappool.beatmaps.find(
                  (m) => m.beatmap_id == mapid || m.md5 == md5
              ) ?? { id: data.menu.bm.id, mods: "NM", identifier: null }
            : { id: null, mods: "NM", identifier: null };
        const mods = map.mods ?? "NM";
        const stats = getModStats(
            data.menu.bm.stats.memoryCS,
            data.menu.bm.stats.memoryAR,
            data.menu.bm.stats.memoryOD,
            data.menu.bm.stats.BPM.common,
            mods
        );
    ...
    if (image !== data.menu.bm.path.full) {
        ...
        $("#map_image").css(
            "background-image",
            `url('http://${location.host}/Songs/${data.menu.bm.path.full}')`
        );
    }
    ...
    if (scoreVisible) {
        ...
        const lead_bar_width = `${Math.max(
            10,
            360 * (Math.min(0.5, Math.pow(scorediff / 1000000, 0.7)) * 2)
        )}px`;
    ...
    if (chatLen !== data.tourney.manager.chat.length) {
        ...
        if (chatLen === 0 || (chatLen > 0 && chatLen > current_chat_len)) {
            $("#chat").html("");
            chatLen = 0;
        }
        ...
        for (let i = chatLen; i < current_chat_len; i++) {
            ...
            if (body.toLowerCase().startsWith("!mp")) {
                ...
                if (player === "BanchoBot" && body.startsWith("Match history"))
                    continue;

                team_actual = teams.find((t) =>
                    t.players.map((p) => p.username).includes(player)
                )?.team;
                teamcode_actual = team_actual
                    ? team_actual === nameRed
                        ? "red"
                        : team_actual === nameBlue
                        ? "blue"
                        : null
                    : null;
                ...
                chatParent.append(
                    $("<div></div>").addClass("chat-time").text(time)
                );
                chatParent.append(
                    $("<div></div>").addClass("chat-name").text(player)
                );
                chatParent.append(
                    $("<div></div>").addClass("chat-body").text(body)
                );
...
const really_start_timer = (length) => {
    ...
    if (length > 30) {
        blink_timer = setTimeout(() => {
            ...
            $("#timer_progress").css(
                "animation",
                `progress ${length}s linear, progress_blink 0.7s infinite ease`
            );
            $("#stopwatch_container").css(
                "animation",
                `progress_blink 0.7s infinite ease`
            );
        }, (length - 3) * 1000);
    }
