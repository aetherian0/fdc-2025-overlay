...
(async () => {
    ...
    if (mappool)
        $("#stage_name")
            .html(`${stage.stage} MAPPOOL SHOWCASE`)
            .css("opacity", 1);
...
socket.onmessage = async (event) => {
    ...
    if (update_stats) {
        ...
        const map = mappool.find(
            (m) => m.beatmap_id === data.beatmap.id || m.md5 === cache.md5
        );
        ...
        $("#sr").html(
            (map?.sr || data.beatmap.stats.stars.total).toFixed(2) + "★"
        );
        ...
        if (map?.beatmapset_id) {
            $("#map_background").css(
                "background-image",
                `url('https://assets.ppy.sh/beatmaps/${map?.beatmapset_id}/covers/cover@2x.jpg')`
            );
            $("#map_stats_background").css(
                "background-image",
                `url('https://assets.ppy.sh/beatmaps/${map?.beatmapset_id}/covers/cover@2x.jpg')`
            );
        } else {
        ...
        if (map?.custom) {
            $("#custom_mapper").text(data.beatmap.mapper);
